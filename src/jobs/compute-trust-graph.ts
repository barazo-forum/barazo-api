...
import type {
  ...
  BehavioralFlag,
} from '../services/behavioral-heuristics.js'
...
export function createTrustGraphJob(
  ...
  async function run(communityId: string | null): Promise<JobResult> {
    ...
    try {
      ...
      const trustComputation = await trustGraphService.computeTrustScores(communityId)
      ...
      const behavioralFlags = await behavioralHeuristicsService.runAll(communityId)
      ...
      const sybilDetection = await sybilDetectorService.detectClusters(communityId)
      ...
      logger.info({ communityId, durationMs }, 'Trust graph computation job completed')
      ...
      logger.error({ communityId, err }, 'Trust graph computation job failed')
