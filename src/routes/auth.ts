 ...
 */
export function authRoutes(oauthClient: NodeOAuthClient): FastifyPluginCallback {
  return (app, _opts, done) => {
    ...
    app.get(
      ...
      async (request, reply) => {
        ...
        const requestedScope = crosspost === 'true' ? BARAZO_CROSSPOST_SCOPES : BARAZO_BASE_SCOPES
    ...
    app.get(
      ...
      async (request, reply) => {
        ...
        try {
          ...
          const grantedScope =
            (result.session as { tokenSet?: { scope?: string } }).tokenSet?.scope ?? ''
