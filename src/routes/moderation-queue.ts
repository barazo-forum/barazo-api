...
export function moderationQueueRoutes(): FastifyPluginCallback {
  return (app, _opts, done) => {
    ...
    const requireModerator = createRequireModerator(
      db,
      authMiddleware,
      app.log,
    );
    ...
    app.put("/api/moderation/queue/:id", {
      preHandler: [requireModerator],
      schema: {
        tags: ["Moderation"],
      ...
      const existing = await db
        ...
        .where(
          and(
            eq(moderationQueue.id, queueId),
            eq(moderationQueue.communityDid, communityDid),
          ),
        );
      ...
      await db.transaction(async (tx) => {
        ...
        if (action === "approve") {
          ...
          const settingsRows = await tx
            .select({
              moderationThresholds:
                communitySettings.moderationThresholds,
            })
          ...
          if (existingTrust.length > 0) {
            const newCount =
              (existingTrust[0]?.approvedPostCount ?? 0) + 1;
            ...
            await tx
              ...
              .set({
                ...
                ...(nowTrusted && !existingTrust[0]?.isTrusted
                  ? { trustedAt: new Date() }
                  : {}),
              })
              .where(
    ...
    app.get("/api/admin/moderation/word-filter", {
    ...
    app.put("/api/admin/moderation/word-filter", {
      preHandler: [requireAdmin],
      schema: {
        tags: ["Admin"],
