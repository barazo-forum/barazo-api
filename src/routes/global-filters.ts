...
export function globalFilterRoutes(): FastifyPluginCallback {
  return (app, _opts, done) => {
    ...
    app.get(
      ...
      async (request, reply) => {
        ...
        if (hasMore) {
          ...
          if (lastRow) {
            nextCursor = encodeCursor(lastRow.updatedAt.toISOString(), lastRow.communityDid)
          }
        }
    ...
    app.put(
      '/api/global/filters/communities/:did',
      {
        preHandler: [requireOperator],
        schema: {
          tags: ['Global Filters'],
    ...
    app.put(
      '/api/global/filters/accounts/:did',
      {
        preHandler: [requireOperator],
        schema: {
          tags: ['Global Filters'],
      ...
      async (request, reply) => {
        ...
        app.log.info({ accountDid: did, status, operatorDid: user.did }, 'Account filter updated')
    ...
    app.get(
      '/api/global/reports/communities',
      {
        preHandler: [requireOperator],
        schema: {
          tags: ['Global Filters'],
