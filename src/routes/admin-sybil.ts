...
function triggerRecompute(app: { trustGraphService: { computeTrustScores(communityId: string | null): Promise<unknown> }; log: { warn(obj: unknown, msg: string): void; info(msg: string): void } }): void {
  ...
  return (app, _opts, done) => {
    ...
    app.get("/api/admin/trust-seeds", {
      ...
      const implicitUsers = await db
        ...
        .from(users)
        .where(
          sql`${users.role} IN ('admin', 'moderator')`,
        );
      ...
      return reply.status(200).send({
        seeds: [
          ...resultRows.map((r) => serializeTrustSeed(r, false)),
          ...implicitSeeds,
        ],
        cursor: nextCursor,
      });
    ...
    app.post("/api/admin/trust-seeds", {
      preHandler: [requireAdmin],
      schema: {
        tags: ["Admin - Sybil"],
      ...
      app.log.info(
        { seedId: seed.id, did, addedBy: admin.did },
        "Trust seed added",
      );
      ...
      return reply.status(201).send(serializeTrustSeed(
    ...
    app.delete("/api/admin/trust-seeds/:id", {
      preHandler: [requireAdmin],
      schema: {
        tags: ["Admin - Sybil"],
    ...
    app.get("/api/admin/sybil-clusters", {
      preHandler: [requireAdmin],
      schema: {
        tags: ["Admin - Sybil"],
    ...
    app.get("/api/admin/sybil-clusters/:id", {
      preHandler: [requireAdmin],
      schema: {
        tags: ["Admin - Sybil"],
    ...
    app.put("/api/admin/sybil-clusters/:id", {
      preHandler: [requireAdmin],
      schema: {
        tags: ["Admin - Sybil"],
    ...
    app.post("/api/admin/trust-graph/recompute", {
      preHandler: [requireAdmin],
      schema: {
        tags: ["Admin - Sybil"],
    ...
    app.get("/api/admin/trust-graph/status", {
      preHandler: [requireAdmin],
      schema: {
        tags: ["Admin - Sybil"],
      ...
      const [nodeRows, edgeRows, flaggedRows] = await Promise.all([
        ...
        db.select({ flaggedCount: count() }).from(sybilClusters).where(eq(sybilClusters.status, "flagged")),
      ]);
    ...
    app.get("/api/admin/behavioral-flags", {
      preHandler: [requireAdmin],
      schema: {
        tags: ["Admin - Sybil"],
        ...
        querystring: {
          type: "object",
          properties: {
            flagType: { type: "string", enum: ["burst_voting", "content_similarity", "low_diversity"] },
    ...
    app.put("/api/admin/behavioral-flags/:id", {
      preHandler: [requireAdmin],
      schema: {
        tags: ["Admin - Sybil"],
      ...
      app.log.info(
        { flagId, status: parsed.data.status },
        "Behavioral flag status updated",
      );
