  ...
  return (app, _opts, done) => {
    ...
    app.get(
      ...
      async (request, reply) => {
        ...
        const rows = await db
          .select()
          .from(categories)
          .where(and(eq(categories.slug, slug), eq(categories.communityDid, communityDid)))
        ...
        const topicCountResult = await db
          .select({ count: count() })
          .from(topics)
          .where(and(eq(topics.category, slug), eq(topics.communityDid, communityDid)))
    ...
    app.post(
      ...
      async (request, reply) => {
        ...
        if (parentId !== undefined) {
          const parentRows = await db.select().from(categories).where(eq(categories.id, parentId))
        ...
        app.log.info({ categoryId: id, slug, adminDid: request.user?.did }, 'Category created')
      ...
      async (request, reply) => {
        ...
        if (updates.slug !== undefined && updates.slug !== existing.slug) {
          const existingSlug = await db
            ...
            .from(categories)
            .where(
              and(eq(categories.slug, updates.slug), eq(categories.communityDid, communityDid))
            )
    ...
    app.delete(
      ...
      async (request, reply) => {
        ...
        const topicCountResult = await db
          .select({ count: count() })
          .from(topics)
          .where(and(eq(topics.category, existing.slug), eq(topics.communityDid, communityDid)))
        ...
        const childRows = await db.select().from(categories).where(eq(categories.parentId, id))
    ...
    app.put(
      '/api/admin/categories/:id/maturity',
      {
        preHandler: [requireAdmin],
        schema: {
          tags: ['Categories (Admin)'],
