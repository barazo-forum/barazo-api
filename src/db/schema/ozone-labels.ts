import { pgTable, text, boolean, timestamp, index, serial, uniqueIndex } from 'drizzle-orm/pg-core'
...
export const ozoneLabels = pgTable(
  ...
  (table) => [
    uniqueIndex('ozone_labels_src_uri_val_idx').on(table.src, table.uri, table.val),
