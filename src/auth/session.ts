...
export function createSessionService(
  ...
  async function createSession(did: string, handle: string): Promise<SessionWithToken> {
    ...
    try {
      ...
      await cache.set(`${SESSION_DATA_PREFIX}${sid}`, JSON.stringify(session), 'EX', sessionTtl)

      // Store access token hash → session ID mapping (TTL = access token lifetime)
      await cache.set(`${ACCESS_TOKEN_PREFIX}${tokenHash}`, sid, 'EX', accessTokenTtl)
      ...
      logger.debug({ did, sid: truncateForLog(sid) }, 'Session created')

      // Return with raw token for the HTTP response (never persisted)
      return { ...session, accessToken }
    } catch (err: unknown) {
      logger.error({ err, did, sid: truncateForLog(sid) }, 'Failed to create session')
      throw err
    }
  }
  ...
  async function validateAccessToken(accessToken: string): Promise<Session | undefined> {
    ...
    try {
      ...
      if (sid === null) {
        logger.debug({ tokenHash: truncateForLog(tokenHash) }, 'Access token not found')
        return undefined
      }
      ...
      logger.error({ err, tokenHash: truncateForLog(tokenHash) }, 'Failed to validate access token')
      throw err
    }
  }
  ...
  async function refreshSession(sid: string): Promise<SessionWithToken | undefined> {
    try {
      ...
      const data = await cache.get(`${SESSION_DATA_PREFIX}${sid}`)
      if (data === null) {
        logger.debug({ sid: truncateForLog(sid) }, 'Session not found for refresh')
        return undefined
      }
      ...
      await cache.set(`${ACCESS_TOKEN_PREFIX}${newTokenHash}`, sid, 'EX', accessTokenTtl)

      // Update session data (sliding window: resets TTL on refresh)
      await cache.set(`${SESSION_DATA_PREFIX}${sid}`, JSON.stringify(updated), 'EX', sessionTtl)

      logger.debug({ sid: truncateForLog(sid) }, 'Session refreshed')

      // Return with raw token for the HTTP response (never persisted)
      return { ...updated, accessToken: newAccessToken }
    } catch (err: unknown) {
      logger.error({ err, sid: truncateForLog(sid) }, 'Failed to refresh session')
      throw err
    }
  }
  ...
  async function deleteSession(sid: string): Promise<void> {
    try {
      ...
      const data = await cache.get(`${SESSION_DATA_PREFIX}${sid}`)
      if (data === null) {
        logger.debug({ sid: truncateForLog(sid) }, 'Session not found for deletion')
        return
      }
      ...
      logger.debug({ sid: truncateForLog(sid) }, 'Session deleted')
    } catch (err: unknown) {
      logger.error({ err, sid: truncateForLog(sid) }, 'Failed to delete session')
      throw err
    }
  }
  ...
  async function deleteAllSessionsForDid(did: string): Promise<number> {
    try {
      ...
      logger.debug({ did, count: sids.length }, 'All sessions deleted for DID')

      return sids.length
    } catch (err: unknown) {
      logger.error({ err, did }, 'Failed to delete all sessions for DID')
      throw err
    }
  }
