    ...
    try {
      ...
      if (rows.length > 0) {
        ...
        const detailParts = rows.map(
          (r) => `${r.authorDid}: ${String(r.reactionCount)} reactions in ${String(BURST_WINDOW_MINUTES)}min`,
        );
  ...
  async function detectLowDiversity(communityId: string | null): Promise<BehavioralFlag[]> {
    ...
    try {
      ...
      if (rows.length > 0) {
        const affectedDids = rows.map((r) => r.authorDid);
        const detailParts = rows.map(
          (r) => `${r.authorDid}: ${String(r.totalInteractions)} interactions, ${String(r.uniqueTargets)} unique targets`,
        );
