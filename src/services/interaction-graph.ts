...
export function createInteractionGraphService(
  ...
  async function recordReply(
    ...
    logger.debug(
      { replierDid, topicAuthorDid, communityId },
      "Recorded reply interaction",
    );
  }
  ...
  async function recordReaction(
    ...
    logger.debug(
      { reactorDid, contentAuthorDid, communityId },
      "Recorded reaction interaction",
    );
  }

  async function recordCoParticipation(
    topicUri: string,
    communityId: string,
  ): Promise<void> {
    ...
    // Deduplicate
    const uniqueAuthors = [
      ...new Set(authorRows.map((r) => r.authorDid)),
    ];

    // Skip if too many authors or not enough for pairs
    if (
      uniqueAuthors.length > MAX_COPARTICIPATION_AUTHORS ||
