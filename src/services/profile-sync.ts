  ...
  return {
    async syncProfile(did: string): Promise<ProfileData> {
      ...
      try {
        ...
        logger.debug(
          { did, err },
          "profile sync failed: could not restore OAuth session",
        );
        return NULL_PROFILE;
      }
      ...
      try {
        ...
        logger.debug(
          { did, err },
          "profile sync failed: could not fetch profile from PDS",
        );
        return NULL_PROFILE;
      }
      ...
      try {
        ...
        logger.warn(
          { did, err },
          "profile DB update failed: could not persist profile data",
        );
      }
