...
export interface TrustGraphService {
  computeTrustScores(communityId: string | null): Promise<TrustComputationResult>
...
export function createTrustGraphService(db: Database, logger: Logger): TrustGraphService {
  async function computeTrustScores(communityId: string | null): Promise<TrustComputationResult> {
    const start = Date.now()

    // 1. Load interaction graph edges
    const communityFilter = communityId ? eq(interactionGraph.communityId, communityId) : sql`true`
    ...
    if (edgeRows.length === 0) {
      logger.info({ communityId }, 'No edges found, skipping trust computation')
      return {
        totalNodes: 0,
        totalEdges: 0,
        iterations: 0,
    ...
    const seedFilter = communityId
      ? or(eq(trustSeeds.communityId, communityId), eq(trustSeeds.communityId, ''))
      : eq(trustSeeds.communityId, '')

    const seedRows = await db.select({ did: trustSeeds.did }).from(trustSeeds).where(seedFilter)
    ...
    // 3. Run EigenTrust
    const result = runEigenTrust(edges, seedDids, MAX_ITERATIONS, CONVERGENCE_THRESHOLD, true)
  ...
  async function getTrustScore(did: string, communityId: string | null): Promise<number> {
    const effectiveCommunityId = communityId ?? ''
    const filter = and(eq(trustScores.did, did), eq(trustScores.communityId, effectiveCommunityId))

    const rows = await db.select({ score: trustScores.score }).from(trustScores).where(filter)
