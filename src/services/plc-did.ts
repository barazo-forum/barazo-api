...
export function compressedPubKeyToDidKey(pubKey: Uint8Array): string {
  ...
  const prefixed = new Uint8Array(SECP256K1_MULTICODEC_PREFIX.length + pubKey.length)
 ...
 */
export function computeDidFromSignedOperation(signedOp: SignedPlcOperation): string {
...
async function submitToPlcDirectory(
  ...
  if (!response.ok) {
    ...
    logger.error({ did, status: response.status, body }, 'PLC directory rejected genesis operation')
    throw new Error(`PLC directory returned ${String(response.status)}: ${body}`)
  }
...
export function createPlcDidService(logger: Logger): PlcDidService {
  async function generateDid(params: GenerateDidParams): Promise<GenerateDidResult> {
    const plcDirectoryUrl = params.plcDirectoryUrl ?? DEFAULT_PLC_DIRECTORY_URL
