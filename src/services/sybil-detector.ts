...
export function createSybilDetectorService(db: Database, logger: Logger): SybilDetectorService {
  async function detectClusters(communityId: string | null): Promise<DetectionResult> {
    ...
    const trustFilter = communityId
      ? and(
          ...
          or(eq(trustScores.communityId, communityId), eq(trustScores.communityId, ''))
        )
      : and(lt(trustScores.score, LOW_TRUST_THRESHOLD), eq(trustScores.communityId, ''))
    ...
    if (lowTrustDids.size === 0) {
      logger.info({ communityId }, 'No low-trust DIDs found, skipping sybil detection')
      return {
        clustersDetected: 0,
        totalLowTrustDids: 0,
    ...
    for (const cluster of clusterInfos) {
      ...
      const clusterHash = createHash('sha256').update(sortedMembers.join(',')).digest('hex')
      ...
      const median = connectionCounts[Math.floor(connectionCounts.length / 2)] ?? 0

      // Delete existing members and re-insert
      await db.delete(sybilClusterMembers).where(eq(sybilClusterMembers.clusterId, clusterId))
