...
describe("firehose account deletion (integration)", () => {
  ...
  it("purges all data for deleted account", async () => {
    ...
    const topicsBefore = await db
      .select()
      .from(topics)
      .where(eq(topics.authorDid, deletedUserDid));
    ...
    const userBefore = await db
      .select()
      .from(users)
      .where(eq(users.did, deletedUserDid));
  ...
  it("preserves other users' data during deletion", async () => {
    ...
    const survivingTopics = one(
      await db
        .select()
        .from(topics)
        .where(eq(topics.authorDid, survivingUserDid)),
    );
    expect(survivingTopics.title).toBe("Surviving user topic");

    // Verify surviving user still exists
    const survivingUser = one(
      await db
        .select()
        .from(users)
        .where(eq(users.did, survivingUserDid)),
    );
    ...
    const survivingReactions = one(
      await db
        .select()
        .from(reactions)
        .where(eq(reactions.authorDid, survivingUserDid)),
    );
  ...
  it("handles active identity event (user handle update)", async () => {
    ...
    const userBefore = one(
      await db
        .select()
        .from(users)
        .where(eq(users.did, "did:plc:handle-test")),
    );
