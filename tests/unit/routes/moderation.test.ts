...
async function buildTestApp(user?: RequestUser, adminUserObj?: RequestUser): Promise<FastifyInstance> {
...
describe("moderation routes", () => {
  ...
  describe("GET /api/moderation/log", () => {
    ...
    it("returns paginated moderation actions", async () => {
      ...
      expect(response.statusCode).toBe(200);
      const body = response.json<{ actions: Array<{ id: number; action: string; createdAt: string }>; cursor: string | null }>();
  ...
  describe("POST /api/moderation/report", () => {
    ...
    it("creates a report successfully", async () => {
      ...
      const body = response.json<{ id: number; reporterDid: string; targetUri: string; reasonType: string; status: string }>();
  ...
  describe("GET /api/moderation/reports", () => {
    ...
    it("returns paginated reports", async () => {
      ...
      expect(response.statusCode).toBe(200);
      const body = response.json<{ reports: Array<{ id: number; status: string; createdAt: string }>; cursor: string | null }>();
  ...
  describe("PUT /api/moderation/reports/:id", () => {
    ...
    it("resolves a pending report", async () => {
      ...
      expect(response.statusCode).toBe(200);
      const body = response.json<{ id: number; status: string; resolutionType: string; resolvedBy: string }>();
      expect(body.id).toBe(1);
