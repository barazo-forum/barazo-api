...
function createMockAuthMiddleware(user?: RequestUser) {
  return {
    requireAuth: async (request: { user?: RequestUser }, reply: { sent: boolean; status: (code: number) => { send: (body: unknown) => Promise<void> } }) => {
      if (!user) {
        await reply.status(401).send({ error: "Authentication required" });
        return;
      }
  ...
  describe("GET /api/admin/trust-seeds", () => {
    ...
    it("returns paginated list of trust seeds with implicit seeds", async () => {
      ...
      const implicitChain = createChainableProxy([
        { did: "did:plc:mod001", handle: "mod.bsky.social", displayName: "Mod", role: "moderator", firstSeenAt: new Date(TEST_NOW) },
      ]);

      mockDb.select
        .mockReturnValueOnce(explicitChain)
        .mockReturnValueOnce(implicitChain);
  ...
  describe("GET /api/admin/sybil-clusters/:id", () => {
    ...
    it("returns cluster detail with enriched members", async () => {
      ...
      const membersChain = createChainableProxy([
        {
          did: "did:plc:member1", roleInCluster: "core", joinedAt: new Date(TEST_NOW),
          handle: "member1.bsky.social", displayName: "Member One",
          reputationScore: 50, accountCreatedAt: new Date(TEST_NOW), trustScore: 0.8,
        },
        {
          did: "did:plc:member2", roleInCluster: "peripheral", joinedAt: new Date(TEST_NOW),
          handle: null, displayName: null,
          reputationScore: 10, accountCreatedAt: null, trustScore: null,
        },
      ]);

      mockDb.select
        .mockReturnValueOnce(clusterChain)
      ...
      const body = response.json<{
        ...
        members: Array<{ did: string; roleInCluster: string; handle: string | null; trustScore: number | null }>;
      }>();
