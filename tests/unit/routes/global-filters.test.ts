...
import { type DbChain, createChainableProxy, createMockDb, type MockDb } from "../../helpers/mock-db.js";
...
describe("global filter routes", () => {
  ...
  describe("GET /api/global/filters/communities", () => {
    ...
    it("returns community filters with serialized dates", async () => {
      ...
      expect(response.statusCode).toBe(200);
      const body = response.json<{ filters: Array<Record<string, unknown>>; cursor: string | null }>();
  ...
  describe("GET /api/global/reports/communities", () => {
    ...
    it("handles communities with zero topic counts", async () => {
      const rows = [
        { community_did: "did:plc:community1", report_count: 3, topic_count: 0 },
      ];
      mockDb.execute.mockResolvedValueOnce(rows);
