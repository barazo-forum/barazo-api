...
describe('upload routes', () => {
  ...
  describe('POST /api/communities/:communityDid/profile/avatar', () => {
    ...
    it('uploads avatar and returns URL', async () => {
      ...
      const { body, contentType } = createMultipartPayload('avatar.png', 'image/png', imageData)
      ...
      expect(result.url).toBe('http://localhost:3000/uploads/avatars/test.webp')
  ...
  describe('POST /api/communities/:communityDid/profile/banner', () => {
    ...
    it('uploads banner and returns URL', async () => {
      const imageData = Buffer.from('fake-png-data')
      const { body, contentType } = createMultipartPayload('banner.png', 'image/png', imageData)
      ...
      expect(response.statusCode).toBe(200)
      const result = response.json<{ url: string }>()
      expect(result.url).toBe('http://localhost:3000/uploads/banners/test.webp')
      // eslint-disable-next-line @typescript-eslint/unbound-method
      expect(mockStorage.store).toHaveBeenCalledOnce()
      expect(mockDb.insert).toHaveBeenCalledOnce()
    })
