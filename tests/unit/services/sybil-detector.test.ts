...
function makeChain(result: unknown = []) {
  ...
  chain.onConflictDoUpdate = vi
    .fn()
    .mockImplementation(() => ({ then: thenFn, returning: returningFn }))
...
describe('SybilDetectorService', () => {
  ...
  describe('detectClusters', () => {
    ...
    it('should skip components with fewer than 3 members', async () => {
      const lowTrustRows = [{ did: 'did:sybil1' }, { did: 'did:sybil2' }]

      const subgraphRows = [{ source_did: 'did:sybil1', target_did: 'did:sybil2', weight: 5 }]
