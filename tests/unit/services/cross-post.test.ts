...
describe('cross-post service', () => {
  ...
  beforeEach(() => {
    ...
    mockDb.select.mockReturnValueOnce(scopeSelectChain).mockReturnValue(selectChain)
  ...
  describe('crossPostTopic', () => {
    it('cross-posts to Bluesky when enabled', async () => {
      ...
      expect((record.embed as Record<string, unknown>).$type).toBe('app.bsky.embed.external')
    ...
    it('notifies user when Bluesky cross-post fails', async () => {
      mockPds.createRecord.mockRejectedValue(new Error('Bluesky PDS unreachable'))
    ...
    it('notifies user when Frontpage cross-post fails', async () => {
      mockPds.createRecord.mockRejectedValue(new Error('Frontpage PDS error'))
    ...
    it('notifies for each failed service independently', async () => {
      mockPds.createRecord.mockRejectedValue(new Error('PDS error'))
  ...
  describe('deleteCrossPosts', () => {
    ...
    it('deletes all cross-posts for a topic from PDS and DB', async () => {
      ...
      expect(mockPds.deleteRecord).toHaveBeenCalledWith(TEST_DID, 'app.bsky.feed.post', 'bsky001')
      expect(mockPds.deleteRecord).toHaveBeenCalledWith(TEST_DID, 'fyi.frontpage.post', 'fp001')
    ...
    it('cleans up DB rows even when PDS delete fails', async () => {
      ...
      mockPds.deleteRecord.mockRejectedValue(new Error('PDS delete failed'))
