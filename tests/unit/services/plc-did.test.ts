...
describe("PlcDidService", () => {
  ...
  beforeEach(() => {
    ...
    mockFetch = vi.fn<typeof globalThis.fetch>().mockResolvedValue(
  ...
  it("submits to custom plc directory URL when specified", async () => {
    ...
    expect(url).toMatch(
      /^https:\/\/plc\.test\.local\/did:plc:[a-z2-7]{24}$/,
    );
  });
  ...
  it("sends a valid signed operation in request body", async () => {
    ...
    expect(
      (body.verificationMethods as Record<string, string>).atproto,
    ).toMatch(/^did:key:z/);
  ...
  it("throws when plc.directory returns non-200 status", async () => {
    mockFetch.mockResolvedValueOnce(
      new Response("Invalid operation", { status: 400 }),
    );
