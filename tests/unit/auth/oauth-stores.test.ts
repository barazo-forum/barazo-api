...
describe('ValkeyStateStore', () => {
  ...
  describe('set', () => {
    ...
    it('logs error and rethrows on cache failure', async () => {
      ...
      await expect(store.set('abc123', mockState)).rejects.toThrow('Valkey connection refused')
...
describe('ValkeySessionStore', () => {
  ...
  describe('set', () => {
    ...
    it('logs error and rethrows on cache failure', async () => {
      ...
      await expect(store.set('did:plc:test', mockSession)).rejects.toThrow('Valkey write error')
      expect(errorFn).toHaveBeenCalledWith(
  ...
  describe('get', () => {
    ...
    it('returns deserialized session when found', async () => {
      ...
      expect(getFn).toHaveBeenCalledWith('barazo:oauth:session:did:plc:test-user-123')
    })
    ...
    it('logs error and rethrows on cache failure', async () => {
      ...
      await expect(store.get('did:plc:test')).rejects.toThrow('Valkey read error')
    })
  })
  ...
  describe('del', () => {
    it('deletes with correct key prefix', async () => {
      ...
      expect(delFn).toHaveBeenCalledWith('barazo:oauth:session:did:plc:test-user-123')
    })
    ...
    it('logs error and rethrows on cache failure', async () => {
      ...
      await expect(store.del('did:plc:test')).rejects.toThrow('Valkey delete error')
    })
  })
  ...
  describe('JSON serialization', () => {
    it('round-trips session data correctly through JSON', async () => {
      ...
      setFn.mockImplementation((_key: unknown, value: unknown) => {
