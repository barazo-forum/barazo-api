...
describe('checkWordFilter', () => {
  ...
  it('does NOT match partial words', () => {
    const result = checkWordFilter('this is unspammy content', undefined, ['spam'])
    expect(result.matches).toBe(false)
  })

  it('matches multiple words', () => {
    const result = checkWordFilter('spam and scam content', undefined, ['spam', 'scam', 'fraud'])
    expect(result.matches).toBe(true)
    expect(result.matchedWords).toContain('spam')
  ...
  it('handles special regex characters in filter words', () => {
    ...
    const result = checkWordFilter('visit site.com today', undefined, ['site.com'])
    expect(result.matches).toBe(true)
    // The dot is escaped so "siteXcom" should NOT match
    const result2 = checkWordFilter('visit siteXcom today', undefined, ['site.com'])
...
describe('isNewAccount', () => {
  ...
  it('returns true when no trust record exists', async () => {
    ...
    const result = await isNewAccount(mockDb as never, 'did:plc:user1', 'did:plc:community1', 7)
    expect(result).toBe(true)
  })

  it('returns false when newAccountDays is 0 (disabled)', async () => {
    const result = await isNewAccount(mockDb as never, 'did:plc:user1', 'did:plc:community1', 0)
