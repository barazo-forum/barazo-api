...
describe("resolveAuthors", () => {
  ...
  it("resolves profiles from users table with no community context", async () => {
    const db = createMockDb(
      [
        { did: didAlice, handle: "alice.bsky.social", displayName: "Alice", avatarUrl: "https://cdn.example.com/alice.jpg", bannerUrl: null, bio: null },
        { did: didBob, handle: "bob.bsky.social", displayName: null, avatarUrl: null, bannerUrl: null, bio: null },
      ],
      [],
    );
  ...
  it("applies community profile overrides when communityDid is provided", async () => {
    const db = createMockDb(
      ...
      [
        { did: didAlice, communityDid, displayName: "Alice in Community", avatarUrl: "https://cdn.example.com/alice-community.jpg", bannerUrl: null, bio: null },
      ],
    );
  ...
  it("deduplicates DIDs before querying", async () => {
    const db = createMockDb(
      [{ did: didAlice, handle: "alice.bsky.social", displayName: "Alice", avatarUrl: null, bannerUrl: null, bio: null }],
      [],
    );
