...
describe('createLocalStorage', () => {
  ...
  it('stores a file and returns a valid URL', async () => {
    ...
    const storage = createLocalStorage(tmpDir, 'http://localhost:3000', mockLogger)

    const data = Buffer.from('fake-image-data')
    const url = await storage.store(data, 'image/webp', 'avatars')

    expect(url).toMatch(/^http:\/\/localhost:3000\/uploads\/avatars\/avatars-[a-f0-9-]+\.webp$/)
  ...
  it('maps MIME types to correct extensions', async () => {
    ...
    const unknownUrl = await storage.store(data, 'application/octet-stream', 'test')
