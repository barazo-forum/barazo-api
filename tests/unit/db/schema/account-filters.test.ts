...
import { accountFilters } from '../../../../src/db/schema/account-filters.js'
import { getTableName, getTableColumns } from 'drizzle-orm'
import { getTableConfig } from 'drizzle-orm/pg-core'

describe('account-filters schema', () => {
  it('should have the correct table name', () => {
    expect(getTableName(accountFilters)).toBe('account_filters')
  })

  it('should have all required columns', () => {
    const columns = getTableColumns(accountFilters)
    ...
    expect(columnNames).toContain('banCount')
  ...
  it('should mark required columns as not null', () => {
    ...
    expect(columns.banCount.notNull).toBe(true)
  ...
  it('should default banCount to 0', () => {
    const columns = getTableColumns(accountFilters)
    expect(columns.banCount.default).toBeDefined()
  })

  it('should have a unique index on (did, communityDid)', () => {
    const config = getTableConfig(accountFilters)
